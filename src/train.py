...
def main(args):
    ...
    print('model to devide and benchmarking starting')
        ...
        for split in ['train', 'val']:
            ...
            for i in range(total_step):
                ...
                if not args.ingrs_only:
                    ...
                    loss_dict['recipe_loss'] = recipe_loss.detach().item()
                    loss_dict['perplexity'] = perplexity.detach().item()
                else:
                    recipe_loss = 0
                ...
                if not args.recipe_only:
                    ...
                    loss_dict['ingr_loss'] = ingr_loss.detach().item()

                    eos_loss = losses['eos_loss']
                    eos_loss = eos_loss.mean()
                    loss_dict['eos_loss'] = eos_loss.detach().item()

                    iou_seq = losses['iou']
                    iou_seq = iou_seq.mean()
                    loss_dict['iou'] = iou_seq.detach().item()

                    card_penalty = losses['card_penalty'].mean()
                    loss_dict['card_penalty'] = card_penalty.detach().item()
                else:
