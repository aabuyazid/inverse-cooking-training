...
class Recipe1MDataset(data.Dataset):
    ...
    def __init__(self, data_dir, aux_data_dir, split, maxseqlen, maxnuminstrs, maxnumlabels, maxnumims,
        ...
        self.dataset = pickle.load(open(os.path.join(aux_data_dir, suff + 'testing_'+split+'.pkl'), 'rb'))
...
#            self.ids.append(i)
 ...
 #           tokens.append('<eoi>')
        ...
        tokens.extend(title)
        # add fake token to separate title from recipe
        tokens_native.append('<eoi>')
        for c in captions:
            tokens_native.extend(c)
            tokens_native.append('<eoi>')
        tokens = np.array(tokens_native)
        del tokens_native
...
#            true_ingr_idxs.append(self.ingrs_vocab(labels[i]))
        true_ingr_idxs_n = []
        for i in range(len(labels)):
            true_ingr_idxs_n.append(self.ingrs_vocab(labels[i]))
        true_ingr_idxs = np.array(true_ingr_idxs_n)
        del true_ingr_idxs_n
...
#        target = torch.Tensor(caption)
        ...
        caption_n = self.caption_to_idxs(tokens, caption)
        caption_n.append(self.instrs_vocab('<end>'))

        caption_n = caption[0:self.maxseqlen]
        caption = np.array(caption_n)
        del caption_n
        target = torch.Tensor(caption)
